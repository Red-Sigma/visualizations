...
using RData, GLMakie, DataFrames
...
begin
	using FileIO
	imgfig = Figure()
	imgax = Axis(imgfig[1,1])
	kepler_img = FileIO.load(GLMakie.assetpath("/home/antoni/Downloads/kepler.gl.png"))
	image!(rotr90(kepler_img))
	#scatterlines!(b01.longitude, b01.latitude)
	imgfig
end

# ╔═╡ fc675bdc-79be-4226-9527-c5a978b2e32e
md"""
# A visualization of the movement of animals in the Elk Island National Park
"""

# ╔═╡ f1040093-2589-452e-b809-0b375abf7986
df_for_tom = load("../data/GPS collar data for Thom--2023-05-10.RData")["df_for_tom"]
...
begin
	function animal_movement(animal, anim_color = :orange)
		fig = Figure()
		# TODO fix ascpect ratio of long and lat
		ax = Axis(fig[1,1], autolimitaspect = 1.6, title = "$(animal[1,:].species) $(animal.animalID[1]) movement")
		...
		scatterlines!(ax, animal.longitude, animal.latitude, color = (anim_color, 0.5), alpha = 0.5)
		save("../plots/$(animal.species[1])-$(animal.animalID[1]).png", fig)
		fig
	end
	#animal_movement(b01, colors[2])
	animal_movement(b01, colors[2])
end
...
const einp_longitude = Dict(
    :min => map(tuple -> tuple[1], einp_map.geometry[1][1][1]) |> minimum,
    :max => map(tuple -> tuple[1], einp_map.geometry[1][1][1]) |> maximum,
)

# ╔═╡ 22216176-b430-47df-b370-4414c0bdcc94
const einp_latitude = Dict(
    :min => map(tuple -> tuple[2], einp_map.geometry[1][1][1]) |> minimum,
    :max => map(tuple -> tuple[2], einp_map.geometry[1][1][1]) |> maximum,
)

# ╔═╡ 4c13960d-1108-4b78-bc36-3593b9c54925
let
	geofig = Figure()
	#geoAxis = GeoAxis(fig[1,1]; dest="+proj=natearth")
	geoAxis = GeoAxis(geofig[1,1], title="Bison Movement";)

	xlims!(geoAxis, einp_longitude[:min], einp_longitude[:max])
	ylims!(geoAxis, einp_latitude[:min], einp_latitude[:max])
	autolimits!(geoAxis)

	poly!(geoAxis, einp_map; colormap=:dense, strokecolor=:black, fill)
	scatter!(b01.longitude, b01.latitude)
	geofig
end
